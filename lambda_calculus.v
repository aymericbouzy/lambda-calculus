...
Fixpoint de_bruijn_aux (x: nat) (l: list term) (i p: nat): term :=
match l with
  | nil => var x
  | s :: r => eq_nat_branch x p (increase_var s i 0) (de_bruijn_aux x r i (S p))
end.

Proposition de_bruijn_aux_terminate : forall (l: list term), forall (x i p: nat), x < p -> de_bruijn_aux x l i p = var x.
Proof.
intro. induction l.
intros. simpl. trivial.
...
Proposition de_bruijn_aux_keeps_close : forall (l: list term), forall (x i j p k: nat), closed_list i l -> k > j + i + x -> closed k (de_bruijn_aux x l j p).
Proof.
intro. induction l.
intros. simpl. omega.
intros. simpl. assert (x = p \/ x <> p). omega. inversion H1.
rewrite eq_branch_real_eq. simpl in IHl. apply (increase_var_keeps_close _ _ _ (S i)). apply closed_implication. inversion H. trivial. omega. omega. trivial.
rewrite neq_branch_real_neq. simpl. apply (IHl _ i). inversion H. trivial. trivial. trivial.
Qed.
...
Lemma absent_vars_in_closed : forall (t: term) (n k: nat), closed n t -> k >= n -> absent_var k t.
Proof.
intro. induction t.
intros. simpl. simpl in H. omega.
intros. simpl. apply (IHt (S n)). simpl in H. trivial. omega.
intros. simpl. inversion H. split. apply (IHt1 n). trivial. trivial. apply (IHt2 n). trivial. trivial.
Qed.

Lemma absent_vars_after_increase : forall (t: term) (n p i: nat), i >= n -> i < n + p -> absent_var i (increase_var t p n).
Proof.
intro. induction t.
intros. simpl. assert (n0 > n \/ n0 <= n). omega. inversion H1.
rewrite gt_branch_real_gt. simpl. omega. trivial.
rewrite leq_branch_real_leq. simpl. omega. trivial.
...
Lemma absent_var_after_aux : forall (l: list term) (n p j k m: nat), k >= p + j \/ k < j -> k <> n -> closed_list p l -> absent_var k (de_bruijn_aux n l j m).
Proof.
intro. induction l.
intros. simpl. omega.
intros. simpl. 
assert (n = m \/ n <> m). omega. inversion H. inversion H2.
rewrite eq_branch_real_eq. assert (k < j \/ k >= j). omega. inversion H5.
apply absent_vars_after_increase. omega. omega.
apply (absent_vars_in_closed _ (p + j)). apply (increase_var_keeps_close _ _ _ p). inversion H1. trivial. omega. omega. omega. trivial.
rewrite neq_branch_real_neq. apply (IHl _ p). omega. trivial. inversion H1. trivial. trivial.
inversion H2. rewrite eq_branch_real_eq. apply absent_vars_after_increase. omega. omega. trivial.
...
Inductive reduce_one: term -> term -> Prop :=
  ...
  | red_beta: forall t u v: term, t = de_bruijn_substitution_list (v :: nil) 0 0 u -> reduce_one (application (lambda u) v) t
.
...
Qed.
(*
Proposition closed_after_substitutions : forall (l: list term) (t: term) (k i j p m: nat), closed j t -> closed_list (m - p) l -> m + j <= k + p -> p <= i -> i <= m -> closed k (de_bruijn_substitution_list l i p t).
Proof.
intro. induction l.
intros. rewrite nil_substitution. apply (closed_implication_generalized _ j). trivial. omega.
intro. induction t. intros.
assert (n = i \/ n < i \/ n > i). omega. inversion H4. simpl. rewrite eq_branch_real_eq. inversion H0. simpl in H. apply (increase_var_keeps_close _ _ _ (m - p)). trivial. omega. omega. omega.
inversion H5.
simpl. rewrite neq_branch_real_neq. apply (de_bruijn_aux_keeps_close _ _ (m - p)). inversion H0. trivial. simpl in H. omega. omega.
simpl. rewrite neq_branch_real_neq. inversion H. assert (de_bruijn_aux n l p (S i) = de_bruijn_substitution_list l (S i) p (var n)). simpl. trivial. rewrite H8. apply (IHl _ _ _ j _ (S m)).
inversion H. simpl. omega. simpl. omega. inversion H0. trivial. trivial. assert (S m - p = S (m - p)). omega. rewrite H11. apply closed_list_implication. trivial. omega. omega.
apply (de_bruijn_aux_keeps_close _ _ (m - p)). inversion H0. trivial. omega. omega. 
intros. simpl. simpl in H. apply (IHt _ _ (S j) _ (S m)). trivial. trivial. omega. 
intros. simpl. inversion H. split. 
...
(*
Proposition de_bruijn_aux_translate : forall (e: environment) (n1 n2 p1 p2: nat), correct_env e -> env_size e >= n1 -> env_size e >= n2 -> n1 + p2 = n2 + p1 -> de_bruijn_aux n1 (environment_translation e) p1 = de_bruijn_aux n2 (environment_translation e) p2.
Proof.
intro. induction e.
intros. inversion H. inversion H0. inversion H1. simpl. trivial.
intros. assert (n1 = p1 \/ n1 > p1 \/ n1 < p1). omega. inversion H3. 
inversion H4. simpl. rewrite eq_branch_real_eq. rewrite eq_branch_real_eq. inversion H. inversion H7. rewrite no_change_increase. rewrite no_change_increase. trivial. inversion H. inversion H7. apply (closed_after_substitutions _ _ _ 0 _ 0).

assert (forall (n p: nat), n = p -> de_bruijn_aux n (environment_translation (Env l e1 e2)) p = de_bruijn_aux n (environment_translation e2) p).
intros. simpl. rewrite eq_branch_real_eq.
Qed.


Lemma krivine_access_Sn_is_reduction : forall (e1: environment) (s1: stack) (e2 : environment) (s2 : stack) (c2 c1: list_instruction) (n: nat),
Some (c2, e2, s2) = one_step_krivine (Block (Access (S n)) c1, e1, s1) ->
correct_state (Block (Access (S n)) c1) e1 s1 ->
...
(Some (c2, e2, s2) = one_step_krivine (Block (Access n) c1, e1, s1) ->
      correct_state (Block (Access n) c1) e1 s1 ->
      reduce_one (state_translation (Block (Access n) c1) e1 s1)
        (state_translation c2 e2 s2) \/
      state_translation (Block (Access n) c1) e1 s1 =
      state_translation c2 e2 s2) ->
reduce_one (state_translation (Block (Access (S n)) c1) e1 s1)
  (state_translation c2 e2 s2) \/
state_translation (Block (Access (S n)) c1) e1 s1 =
state_translation c2 e2 s2.
Proof.
intros. inversion H. induction e1. inversion H4.
inversion H. induction s1. simpl. inversion H0. inversion H3. inversion H10. inversion H12. assert (correct_state c2 e2 s2).
apply (krivine_keeps_correct _ (Block (Access (S n)) c1) (Env l e1_1 e1_2) Nil_stack) . trivial. rewrite H. trivial. inversion H4 in H13. inversion H13. inversion H15. inversion H21. inversion H23.
...
intro. induction s1.
intros. inversion H.
intros. inversion H. simpl. inversion H0. inversion H1. inversion H7. simpl in H9.
assert (correct_state c2 e2 s2). apply (krivine_keeps_correct _ (Block Grab c1) e1 (St l e s1)). trivial. rewrite H. trivial.

 
left. apply red_beta. 
assert (state_translation c1 (Env l e e1) s1 = state_translation c2 e2 s2). inversion H. trivial.


Qed.


Lemma de_bruijn_aux_environment : forall (e: environment) (n p: nat), S n = env_size e -> de_bruijn_aux (S n) (environment_translation e) (S p) = de_bruijn_aux n (environment_translation e) p.
Proof.
intro. induction e.
intros. simpl. inversion H.
intros. simpl.
assert (n = p \/ n <> p). omega. inversion H0. rewrite eq_branch_real_eq. rewrite eq_branch_real_eq. inversion H.
admit. trivial. trivial. rewrite neq_branch_real_neq. rewrite neq_branch_real_neq.

Qed.
*)

Theorem de_bruijn_aux_translate : forall (e: environment) (n p i: nat), env_size e + p > n -> p <= n -> de_bruijn_aux (S n) (environment_translation e) i (S p) = de_bruijn_aux n (environment_translation e) i p.
Proof.
intro. induction e.
intros. simpl in H. contradict H0. omega.
intros. simpl. inversion H0.
rewrite eq_branch_real_eq. rewrite eq_branch_real_eq. trivial. omega. omega.
rewrite neq_branch_real_neq. rewrite neq_branch_real_neq. apply IHe2. simpl in H. omega. omega. omega. omega.
Qed.

Theorem krivine_step_is_reduction: forall (c1: list_instruction) (e1: environment) (s1: stack), Some (c2, e2, s2) = one_step_krivine (c1, e1, s1) -> correct_state c1 e1 s1 -> reduce_one (state_translation c1 e1 s1) (state_translation c2 e2 s2) \/ state_translation c1 e1 s1 = state_translation c2 e2 s2.
Proof.
intros. induction c1.
inversion H.
induction i.
induction n.
right. induction e1. inversion H.
inversion H. induction s1; simpl; rewrite null_increase; trivial.
induction e1. inversion H.
inversion H. right. induction s1. simpl.
rewrite de_bruijn_aux_translate. trivial.
inversion H0. inversion H1. inversion H7. inversion H9. omega. omega. omega.
simpl. rewrite de_bruijn_aux_translate. trivial.
inversion H0. inversion H1. inversion H7. inversion H9. omega. omega. omega.
induction s1. inversion H.
inversion H. simpl.
admit.
admit.
Qed.
